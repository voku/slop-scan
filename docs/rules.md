...
| `php.catch-returns-exception-message` | `catch` blocks that return, assign, or place the caught exception message/string form into generic error payload data | Flattening an exception into ordinary string data blurs success and failure paths, loses type/stack context, and can leak internal details. |
| `php.generic-status-envelopes` | Array literals that pair a boolean `success`/`ok`/`status` key with a generic payload key such as `message`, `error`, `data`, `rows`, or `result`; evidence also records whether the array is returned, assigned, or sent through a JSON response boundary | Wrapping every operation in a shallow boolean envelope describes transport status instead of domain meaning, and is a common shape in generated service glue. |
| `php.generic-array-casts` | `json_decode(..., true)` and `(array)` conversions assigned to vague bag variables such as `$data`, `$payload`, `$parsed`, `$record`, `$result`, or `$config` | Generic runtime conversion into weakly named bags often erases domain structure at exactly the point where a typed DTO or explicit array shape would make intent clearer. |
...
| `php.duplicate-mock-setup` | The same PHPUnit mock/stub behavior setup repeated across three or more test files | Repeated mock plumbing usually belongs in a shared fixture or factory; copied inline it makes every test restate the same wiring instead of the case under test. |
...
For `php.generic-status-envelopes`, the status key must carry a literal `true` or `false`, so `['status' => 'archived', 'message' => $text]` stays quiet. A status key on its own (`['ok' => false]`) and a domain-named payload (`['ok' => true, 'repository' => $repo]`) also stay quiet. Evidence distinguishes returned arrays, arrays passed directly to `->json(...)` or `new JsonResponse(...)`, and other assigned/local arrays. Bundled or generated files whose logical line count exceeds the `maxFileLines` option (default `5000`) are skipped.

For `php.generic-array-casts`, a conversion is only reported when it lands in one of the deliberately vague bag-variable names. A domain variable such as `$user = json_decode($raw, true)` stays quiet, as do `json_decode()` calls that do not explicitly request associative arrays. The rule deliberately does not treat PHPDoc alone as runtime evidence.

For `php.duplicate-mock-setup`, a setup shape only counts once it appears in at least three distinct test files, and it must bind to a PHPUnit mock or stub. Bare `createMock()` / `createStub()` declarations, `tearDown()`-only cleanup, and clusters confined to one or two files stay quiet, so ordinary framework boilerplate does not dominate the signal. The rule is repository-scope: each participating file gets its own finding, and the score grows with the number of files sharing the shape, capped at `5`. It is independent of `php.mock-heavy-tests-without-assertions`, which looks at a single file's mock-to-assertion balance rather than repetition across files.

For `php.catch-returns-exception-message`, direct returns keep the existing evidence shape (`return=caught-message` / `return=caught-string`). The extension only adds assignments to generic error variables and generic error/message array entries, so existing baselines do not change merely because more contexts are now recognized.
