...
import { genericStatusEnvelopesRule } from "../src/rules/generic-status-envelopes";
...
async function createTempRepo(files: Record<string, string>): Promise<string> {
  const rootDir = await mkdtemp(path.join(os.tmpdir(), "slop-scan-generic-status-envelopes-"));
...
describe("generic-status-envelopes rule", () => {
  test("flags shallow boolean-and-payload result wrappers", async () => {
    const rootDir = await createTempRepo({
      "src/slop.ts": [
        ...
        "}",
        "",
        "export function listRows(rows: string[]) {",
        "  return { ok: false, rows };",
        "}",
        "",
        "export function respond(id: string) {",
        "  return Response.json({ ok: true, artifactId: id });",
        "}",
      ].join("\n"),
      "src/legit.ts": [
        "export function createResult(user: object) {",
    ...
    const finding = result.findings.find(
      (nextFinding) => nextFinding.ruleId === "api.generic-status-envelopes",
    );

    expect(finding).toBeDefined();
    expect(finding?.path).toBe("src/slop.ts");
    expect(finding?.evidence).toEqual([
      "line 2: returned-generic-status-envelope",
      "line 10: json-generic-status-envelope",
    ]);
  ...
  test("ignores giant bundled files that would otherwise create vendor noise", async () => {
    const hugeFile = [
      ...
      "export const response = { ok: true, rows: [] };",
      "",
    ].join("\n");
